...
int main(int argc, char **argv) {
    // 1. read source image
    cv::Mat src = cv::imread(argv[1],IMREAD_COLOR);
    ...
    for (int x=0; x<src.cols; ++x) {
        for (int y=0; y<src.rows; ++y) {
            if (dest.at<uchar>(cv::Point(x, y)) == 0) {
                grow(src, dest, mask, cv::Point(x, y), th);
...
void grow(cv::Mat& src, cv::Mat& dest, cv::Mat& mask, cv::Point seed, int th) {
...
//                uchar delta = abs(src.at<uchar>(center) - src.at<uchar>(estimating_point));
