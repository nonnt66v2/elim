...
void drawMerged(TNode *root, Mat &img) {
    ...
    if (!tmp.size()) {
        ...
        if ((root->getMergedB(0) && root->getMergedB(1)) || (root->getMergedB(2) && root->getMergedB(3)))
            rectangle(img, Rect(tmp[0]->getRegion().x, tmp[0]->getRegion().y, tmp[0]->getRegion().height,
                                tmp[0]->getRegion().width * 2), Scalar(0));
        else if ((root->getMergedB(0) && root->getMergedB(3)) || (root->getMergedB(1) && root->getMergedB(2)))
            rectangle(img, Rect(tmp[0]->getRegion().x, tmp[0]->getRegion().y, tmp[0]->getRegion().height * 2,
                                tmp[0]->getRegion().width), Scalar(0));
        if (!root->getMergedB(0))
            drawMerged(root->getUL(), img);
        if (!root->getMergedB(1))
            drawMerged(root->getUR(), img);
...
int main(int argc, char **argv) {
    ...
    segment(root, srcSeg);
//    Mat cannyEdges = srcSeg.clone();
//    Canny(srcSeg,cannyEdges,90,150,3);
